...
export function computeLighting(ctx: GenContext): RobloxLightingSettings {
  ...
  const moodMul: Record<string, { bright: number; sat: number; fog: number; ambientShift: string }> = {
    ...
    moonlit: { bright: 0.55, sat: -0.2, fog: 1.2, ambientShift: "#8fa0d0" },
  ...
  return {
    ...
    brightness,
    ambient: night ? mixHex(ambient, "#101426", 0.35) : ambient,
    outdoorAmbient: night ? mixHex(outdoor, "#1a2040", 0.3) : outdoor,
