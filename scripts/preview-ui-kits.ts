 ...
 *
 *   npx tsx scripts/preview-ui-kits.ts [--out demo-output/ui-kits.html] [--kit pixel_retro]
 *
...
function kitSheet(kit: UiKitDef): string {
  ...
  <section class="card">
    ...
    <div class="window" style="background:${panelBg};border:${stroke}px solid ${edge};border-radius:${s.radius}px;${shadow}opacity:${1 - s.panelTransparency / 2};">
      ${ornamentLayers(kit)}
      <div class="row top">
        <span class="title" style="color:${t.textDark};font-family:${font(s.font)};font-size:${20 * scale}px;${outline}">SHOP</span>
      ...
      <div class="bundle" style="background:linear-gradient(${t.gold[0]}, ${t.gold[1]});border:${stroke}px solid ${edge};border-radius:${Math.min(16, s.radius)}px;color:${onGold}">
        <b style="font-family:${font(s.font)};font-size:${14 * scale}px;${outline}">STARTER PACK</b>
        <span style="font-family:${font(s.fontBody)};font-size:${10 * scale}px">everything you need to start</span>
      ...
      <div class="row bottom">
        ${button("Buy", t.primary, onPrimary)}
        ${button("Equip", t.info, onInfo)}
        ${button("Claim", t.gold, onGold)}
      </div>
    </div>
  </section>`;
}
...
<body>
  <h1>UI kit libraries — ${kits.length} design systems</h1>
  <div class="sub">Generated by <code>scripts/preview-ui-kits.ts</code> from packages/core/src/taxonomy/ui-kits.ts. Browser approximation of src/ui/kit.ts: same tokens, radius, outline, gradients, shadow, bevel, text outline, ornament and label-colour choice.</div>
