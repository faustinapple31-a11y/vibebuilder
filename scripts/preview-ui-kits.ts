...
/** Blends two `#rrggbb` colours (the sheet mirrors the tint rarityEffect applies to a tile). */
  ...
  <section class="card">
    ...
    <div class="window" style="background:${panelBg};border:${stroke}px solid ${edge};border-radius:${s.radius}px;${shadow}opacity:${1 - s.panelTransparency / 2};">
      ...
      <div class="rarity">
        ${[0, 1, 2, 3, 4]
          .map((tier) => {
            const ramp = [t.inkSoft, t.primary[0], t.info[0], "#b48cff", t.gold[0]];
            const c = ramp[tier]!;
            const names = [T.common, T.uncommon, T.rare, T.epic, T.legendary];
            const tiers = ["I", "II", "III", "IV", "V"];
            return `<span class="rar" title="${names[tier]}" style="background:${mixHex(t.tile, c, 0.12 + tier * 0.05)};border:${Math.max(1.5, stroke)}px solid ${c};border-radius:${Math.min(12, s.radius)}px;color:${c};font-family:${font(s.fontBody)};font-size:${9 * scale}px">${tiers[tier]}</span>`;
          })
          .join("")}
      </div>
      <div class="tiles">
...
<style>
  ...
  .rarity { display: flex; gap: 4px; margin-top: 8px; }
  .rar { flex: 1; min-width: 0; text-align: center; padding: 4px 1px; font-weight: 700; text-transform: uppercase; letter-spacing: 0; overflow: hidden; white-space: nowrap; }
  .meta.fx { flex-basis: 100%; text-align: right; color: #6f737c; }
...
<body>
  ...
  <div class="sub">Labels from the ${locale} table (<code>--locale</code> to switch). Generated by <code>scripts/preview-ui-kits.ts</code> from packages/core/src/taxonomy/ui-kits.ts. The rarity strip mirrors <code>rarityFrame</code> (I…V = common → legendary) (epic takes the world's accent colour, a placeholder here). Browser approximation of src/ui/kit.ts: same tokens, radius, outline, gradients, shadow, bevel, text outline, ornament and label-colour choice.</div>
