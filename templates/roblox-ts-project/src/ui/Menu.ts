...
import { body, button, card, prettyName, text, textOn, theme, Window } from "./kit";
...
export class Menu {
	...
	private build(): void {
		...
		text(GameConfig.name, new UDim2(1, -24, 0, 46), new UDim2(0, 12, 0, 12), hero, { size: 34, align: Enum.TextXAlignment.Center, color: textOn(theme.primary[1]), zIndex: 6, scaled: true });
		body(`${prettyName(GameConfig.genre)} · ${prettyName(GameConfig.layout)}`, new UDim2(1, -24, 0, 22), new UDim2(0, 12, 0, 60), hero, { size: 14, align: Enum.TextXAlignment.Center, color: textOn(theme.primary[1]), zIndex: 6 });
