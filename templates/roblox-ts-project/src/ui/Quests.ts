...
import { body, card, coinIcon, panel, progressBar, resourceIcon, sectionHeader, text, theme, Window } from "./kit";
	...
	private render(): void {
		...
		if (done.size() > 0) {
			...
			for (const quest of done) {
				...
				text(quest.title, new UDim2(1, -120, 1, 0), new UDim2(0, 16, 0, 0), row, { size: 22, zIndex: 6 });
				text("✓", new UDim2(0, 60, 1, 0), new UDim2(1, -70, 0, 0), row, { size: 30, align: Enum.TextXAlignment.Center, zIndex: 6 });
				body(`+${quest.reward.amount} ${GameConfig.currency.name}`, new UDim2(0, 200, 0, 18), new UDim2(0, 16, 1, -22), row, { size: 12, color: theme.text, zIndex: 6 });
			}
		}
	}
}
