...
- `src/ui/kits.generated.ts`: **the UI libraries** — 16 complete design systems shipped with the
  ...
  wood & leaf, black & gold, kawaii pastel). `GameConfig.ui.kit` selects one. To change the whole
...
- `kit.theme`: the resolved library — colours (`paper`, `ink`, `primary`, `gold`, `pill`, `tile`,
  ...
  `bevel`, `textOutline`, `panelTransparency`, `ornament`, `press`. Read these instead of hard-coding
